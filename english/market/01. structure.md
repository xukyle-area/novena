...
```
┌─────────────────────────────────────────────────────────┐
│            market-outer (External Interface Layer)      │
│   - WebSocket data ingestion                            │
│   - Spring Boot application                             │
│   - Data writing to Redis/Kafka                         │
└─────────────────┬───────────────────────────────────────┘
                  │
                  ▼ (Kafka Topics)
┌─────────────────────────────────────────────────────────┐
│          market-flink (Stream Processing Core Layer)    │
...
└─────────────────┬───────────────────────────────────────┘
                  ...
                  ▼ (Redis)
┌─────────────────────────────────────────────────────────┐
│           market-common (Common Foundation Layer)       │
...
#### 🔄 market-flink (Flink Job Module)
**Responsibility**: Core stream processing logic, implementing real-time data computation

**Main Jobs**:

##### 1️⃣ OrderbookJob (Order Book Job)
```java
Kafka[order topic] 
  → KeyBy(contractId)
  → OrderBookProcessor (MapState maintains bids/asks)
  → OrderBookSink (write to Redis)
```
...
##### 2️⃣ TickerJob (Market Data Aggregation Job)
```java
Kafka[trade topic]
  → KeyBy(contractId)
  → Window(24h sliding window/1s)
  → TickerAggregator (aggregate trade data)
  → TickerProcessor (generate Ticker)
  → TickerSink (write to Redis)
```
...
#### 🌐 market-outer (External Interface Module)
...
1. **How to efficiently maintain order books?** → Flink MapState + price grouping
2. **How to ensure data consistency?** → Keyed State + exactly-once semantics
3. **How to handle large state issues?** → RocksDB State Backend
4. **How to ensure low latency?** → Async IO + in-memory computation
...
### ✅ Data Flow Direction
External Exchange → WebSocket → market-outer → Kafka → Flink Jobs → Redis → Frontend
