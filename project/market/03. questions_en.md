...
Third, we address **multi-dimensional data aggregation** requirements. The system needs to simultaneously serve different use cases—some users want highly detailed order books with fine resolution, others prefer aggregated views, and everyone wants 24-hour statistics and multi-period candlestick data. We solve this through Flink's ability to run multiple jobs in parallel, each producing different views of the same underlying data stream.
...
**Performance** is the primary factor—Redis operates entirely in memory and can easily handle over 100,000 queries per second, while MySQL, being disk-based, typically maxes out at a few thousand QPS. When you're serving real-time market data to potentially thousands of concurrent users, this performance difference is critical.
...
That said, we do recognize that historical data analysis is valuable, so we asynchronously write aggregated data to MySQL or ClickHouse for offline analytics. This gives us the best of both worlds—Redis handles the hot, real-time data path, while traditional databases store cold data for historical analysis.
...
- **Increase Kafka Partitions**: 16 → 160
- **Increase Flink Parallelism**: 1 → 32
- **Increase TaskManagers**: 1 → 8 servers
...
- Timer optimization: 1s → 100ms
- Incremental updates: only send changed price levels

**Expected Improvement**:
- Single machine 10k TPS → Distributed cluster 100k TPS
...
### ✅ High-Frequency Questions (Focus Preparation)
1. Introduce project
2. Why choose Flink
...
### ✅ Data Metrics (Recommend Memorizing)
